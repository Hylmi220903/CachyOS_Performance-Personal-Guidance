...
case "$ACTION" in
    up|dhcp4-change|reapply)
        ...
        # point this interface to Cloudflare/Quad9 (1.1.1.1/9.9.9.9) so general internet
        ...
        if [[ "$IFACE_IP" =~ ^10\. ]] || [[ "$DHCP4_DOMAIN_NAME_SERVERS" =~ 10\. ]]; then
            if command -v resolvectl >/dev/null 2>&1; then
                resolvectl dns "$IFACE" 1.1.1.1 9.9.9.9 2606:4700:4700::1111 2620:fe::fe 2>/dev/null || true
